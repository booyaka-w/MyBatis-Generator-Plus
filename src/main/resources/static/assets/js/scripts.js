...
! function(r, u) {
	...
	r.ClassBody = function() {
		...
		var t = window.location.href,
			n = (n = t.substring(0, -1 == t.indexOf("#") ? t.length : t.indexOf("#"))).substring(0, -1 == n.indexOf("?") ? n.length :
				n.indexOf("?"));
		u(".nk-menu-link, .menu-link, .nav-link").each(function() {
			var t = u(this),
				e = t.attr("href");
			n.match(e) ? (t.closest("li").addClass("active current-page").parents().closest("li").addClass(
					"active current-page"), t.closest("li").children(".nk-menu-sub").css("display", "block"), t.parents().closest(
					"li").children(".nk-menu-sub").css("display", "block")) : t.closest("li").removeClass("active current-page").parents()
				.closest("li:not(.current-page)").removeClass("active")
		})
	}, r.PassSwitch = function() {
		r.Passcode(".passcode-switch")
	}, r.Toast = function(t, e, n) {
		var i, a = "info" === (e = e || "info") ? "ni ni-info-fill" : "success" === e ? "ni ni-check-circle-fill" : "error" ===
			e ? "ni ni-cross-circle-fill" : "warning" === e ? "ni ni-alert-fill" : "",
			o = {
			...
			s = n ? h(o, n) : o;
		if (s.position = s.position ? "toast-" + s.position : "toast-bottom-right", s.icon = "auto" === s.icon ? a : s.icon ?
			s.icon : "", s.ui = s.ui ? " " + s.ui : "", i = "" !== s.icon ? '<span class="toastr-icon"><em class="icon ' + s.icon +
			'"></em></span>' : "", "" !== (t = "" !== t ? i + '<div class="toastr-text">' + t + "</div>" : "")) {
		...
		r.TGL.screen(i), n.on("click", function(t) {
			...
			if (a) {
				var e = u(a);
				e.is(t.target) || 0 !== e.has(t.target).length || i.is(t.target) || 0 !== i.has(t.target).length || (r.Toggle.removed(
					e.data("target"), s), a = !1)
			}
		}), c.on("resize", function() {
			i.each(function() {
		...
		var n = t || ".nk-menu-toggle",
			...
			a = e ? h(i, e) : i;
		u(n).on("click", function(t) {
			(r.Win.width < p.lg || u(this).parents().hasClass(g) || u(this).parents().hasClass(o)) && r.Toggle.dropMenu(u(
				this), a), t.preventDefault()
		})
		...
		n.on("click", function(t) {
			...
			!n.is(t.target) && 0 === n.has(t.target).length && !i.is(t.target) && 0 === i.has(t.target).length && r.Win.width <
				a && r.Toggle.removed(n.data("target"), s)
		}), c.on("resize", function() {
		...
		i.exists() && (i.on("click", function(t) {
			...
			mouseup: function(t) {
				o.find("input").val() || o.is(t.target) || 0 !== o.has(t.target).length || i.is(t.target) || 0 !== i.has(t.target)
					.length || i.add(o).removeClass(a.active)
			}
		}))
		...
		r.BS.menutip("a.nk-menu-link"), r.BS.tooltip(".nk-tooltip"), r.BS.tooltip(".btn-tooltip", {
				...
				placement: "right"
			}), r.BS.popover('[data-toggle="popover"]'), r.BS.progress("[data-progress]"), r.BS.fileinput(".custom-file-input"),
			r.BS.modalfix(), r.BS.ddfix(), r.BS.tabfix()
		...
		r.coms.docReady.push(r.OtherInit), r.coms.docReady.push(r.Prettify), r.coms.docReady.push(r.ColorBG), r.coms.docReady
			.push(r.ColorTXT), r.coms.docReady.push(r.Copied), r.coms.docReady.push(r.Ani.init), r.coms.docReady.push(r.TGL.init),
			r.coms.docReady.push(r.BS.init), r.coms.docReady.push(r.Validate.init), r.coms.docReady.push(r.Picker.init), r.coms
			.docReady.push(r.Addons.Init)
	}, r.init()
}(NioApp, jQuery);
